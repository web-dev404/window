...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('[data-dimensions]');
...
var images = function images() {
  ...
  imgPopup.appendChild(bigImageDiv);
  imgPopup.style.justifyContent = 'center';
  imgPopup.style.alignItems = 'center';
  imgPopup.style.display = 'none';
  bigImageDiv.style.width = '85%';
  bigImageDiv.style.height = '85%';
  bigImage.style.width = '100%';
  bigImage.style.height = '100%';
  bigImage.classList.add('popupBigImg');
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && (target.classList.contains('popup') || target.classList.contains('popupBigImg'))) {
      imgPopup.style.display = 'none';
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        checkboxCold = document.querySelector('[data-cold]'),
        checkboxWarm = document.querySelector('[data-warm]'),
        scroll = calcScroll();
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        if (item == document.querySelector('.popup_calc_button')) {
          if (inputWidth.value == '' || inputHeight.value == '') {
            if (document.querySelector('.fillTheFormMessage')) {
              return;
            }

            var fillTheForm = document.createElement('div');
            fillTheForm.classList.add('fillTheFormMessage');
            fillTheForm.textContent = 'Заполните все параметры';
            fillTheForm.style.color = 'red';
            document.querySelector('.fillTheForm').append(fillTheForm);
            return;
          }
        }

        if (item == document.querySelector('.popup_calc_profile_button')) {
          if (checkboxCold.checked == false && checkboxWarm.checked == false) {
            if (document.querySelector('.checkTheCheckboxMessage')) {
              return;
            }

            var checkTheCheckbox = document.createElement('div');
            checkTheCheckbox.classList.add('checkTheCheckboxMessage');
            checkTheCheckbox.textContent = 'Заполните все параметры';
            checkTheCheckbox.style.color = 'red';
            document.querySelector('.checkTheCheckbox').append(checkTheCheckbox);
            return;
          }
        }
        ...
        if (document.querySelector('.fillTheFormMessage')) {
          document.querySelector('.fillTheFormMessage').remove();
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    content[i].classList.add('fadeTabs');
